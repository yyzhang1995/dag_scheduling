...
def choose_task_in_queue(queue, rest_working_time, candidates=120):
    """
...
def process_ant(params, send_end):
    ant_num = params['ant_num']

    init_pool = params['init_pool']
    in_degs = params['in_degs']
    mask_available_time_init = params['mask_available_time_init']
    rest_working_time = params['rest_working_time']
    hormones = params['hormone']

    m = params['m']
    T = params['T']
    list_batch = params['list_batch']
    ...
    alpha = params['alpha']
    E = params['E']
    E_trans = params['E_trans']

    proc_num = params['proc_num']

    best_time = float('inf')
    best_working_order = None

    for _ in tqdm(range(ant_num)):
    ...
    np.save(f"../cache/best_task_distribution_proc_{proc_num}.npy", best_task_distribution_this_round)
    with open(f"../cache/best_working_order_{proc_num}.txt", 'w') as f:
        for line in best_working_order:
            f.write(','.join(map(str, line)) + "\n")
    results = {"proc_num": proc_num, "best_time": best_time}
...
def main_loop(params):
    ...
    thread_num = 8
    ant_params = {
        "ant_num": Nant // thread_num,

        "init_pool": init_pool,
        "in_degs": in_degs,
        "mask_available_time_init": mask_available_time_init,
        "rest_working_time": rest_working_time,
        "hormone": hormones,  # 这个需要在每轮结束以后进行更新？
    ...
    for r in range(Nc):
        ...
        process = []
        pipe_list = []
        for proc_num in range(thread_num):
            recv_end, send_end = Pipe(False)
            ant_params_inter = ant_params.copy()
            ant_params_inter["proc_num"] = proc_num
            pro = Process(target=process_ant, args=(ant_params_inter, send_end))
            process.append(pro)
        ...
        result_list = [recv.recv() for recv in pipe_list]

        best_proc = None
        for i in range(thread_num):
            if result_list[i]["best_time"] < best_time:
                best_proc = result_list[i]["proc_num"]
                best_time = result_list[i]["best_time"]

        if best_proc is not None:
            best_task_distribution = np.load(f"../cache/best_task_distribution_proc_{best_proc}.npy")
            with open(f"../cache/best_working_order_{best_proc}.txt", 'r') as f:
                wo = f.readlines()
            best_working_order = [list(map(int, line.strip().split(','))) for line in wo]

        # renew hormones
        hormones = hormones * (1 - rho)
        for i in range(T):
            hormones[i, best_task_distribution[i]] += rho / best_time
